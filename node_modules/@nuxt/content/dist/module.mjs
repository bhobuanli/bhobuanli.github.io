...
async function ensurePackageInstalled(pkg) {
  if (!await isPackageInstalled(pkg)) {
    ...
    const confirm = hasTTY && !isCI ? await logger.prompt(`Do you want to install \`${pkg}\` package?`, {
      type: "confirm",
      name: "confirm",
      initial: true
    }) : false;
...
async function contentsOfPreviousBuild(nuxt, filename) {
  return await readFile(join(nuxt.options.buildDir, filename), "utf8").catch(() => void 0);
}
