...
function resolveRemoteRef(refs, ref) {
  const resolved = ref.split("/").reduce(
    (node, segment) => typeof node === "object" ? node[segment] : void 0,
    refs
  );
...
async function getGitRemoteHash(url, ref) {
  try {
    ...
    if (ref) {
      if (ref.branch) {
        return resolveRemoteRef(remote.refs.heads, ref.branch);
      }
      if (ref.tag) {
        return resolveRemoteRef(remote.refs.tags, ref.tag);
      }
    } else {
      const head = remote.HEAD.replace("refs/heads/", "");
      return resolveRemoteRef(remote.refs.heads, head);
    }
  } catch {
  }
}
...
async function ensurePackageInstalled(pkg) {
  if (!await isPackageInstalled(pkg)) {
    ...
    const confirm = stdEnv.hasTTY && !stdEnv.isCI ? await logger.prompt(`Do you want to install \`${pkg}\` package?`, {
      type: "confirm",
      name: "confirm",
      initial: true
    }) : false;
...
async function contentsOfPreviousBuild(nuxt, filename) {
  return await promises.readFile(pathe.join(nuxt.options.buildDir, filename), "utf8").catch(() => void 0);
}
...
const fullDatabaseCompressedDumpTemplate = (manifest) => ({
  ...
  getContents: async ({ nuxt, options }) => {
...
function watchContents(nuxt, options, manifest) {
  ...
  async function onChange(pathOrError) {
    ...
    const matches = sourceMap.filter(({ source, cwd }) => {
      if (cwd && absolutePath.startsWith(cwd)) {
        return micromatch__default.isMatch(absolutePath.substring(cwd.length), source.include, { ignore: getExcludedSourcePaths(source), dot: true });
      }
      return false;
    });
    if (matches.length) {
  ...
  async function broadcast(collection, key, insert) {
    ...
    if (insert) {
      await Promise.all(insert.queries.map((query) => db2.exec(query)));
    }
  ...
  resolvedSource.prepare = async ({ rootDir }) => {
    ...
    if (typeof source.repository === "object") {
      ...
      if (repository) {
        ...
        const refKey = /^[\w.-]+$/.test(resolvedRef) ? resolvedRef : `${resolvedRef.replace(/[^\w.-]+/g, "-")}-${ohash.hash(resolvedRef).slice(0, 8)}`;
        const refPrefix = source.repository.tag ? "tag-" : "";
        resolvedSource.cwd = pathe.join(rootDir, ".data", "content", `${gitSource}-${owner}-${name}-${refPrefix}${refKey}`);
...
function hubDatabaseToContentDatabase(hubDb) {
  if (hubDb.driver === "d1") {
    return { type: "d1", bindingName: "DB" };
  }
  if (["node-postgres", "postgres-js", "neon-http", "postgres", "postgresql"].includes(hubDb.driver)) {
    return typeof hubDb.connection?.url === "string" && hubDb.connection.url ? { type: "postgresql", url: hubDb.connection.url } : void 0;
  }
  if (["sqlite", "better-sqlite3"].includes(hubDb.driver)) {
    ...
    return filename ? { type: "sqlite", filename } : void 0;
  }
  if (["libsql", "pglite"].includes(hubDb.driver)) {
    return { type: hubDb.driver, ...hubDb.connection };
  }
  return void 0;
}
const nuxthub = definePreset({
  ...
  async setupNitro(nitroConfig, options) {
    ...
    if (nuxt.options.hub?.database === true) {
      ...
      const database2 = hubDatabaseToContentDatabase(hubConfig.db);
      if (database2) {
        nitroConfig.runtimeConfig.content.database ||= database2;
      }
    }
    const database = nitroConfig.runtimeConfig?.content?.database;
    const isRemoteDatabase = database?.type === "d1" || database?.type === "postgresql" || database?.type === "postgres" || database?.type === "libsql" && !!database.url && !database.url.startsWith("file:");
