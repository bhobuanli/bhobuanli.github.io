...
import { t as tryResolveTypesDeclaration, e as extractMacroMeta, c as createMetaChecker, s as stripMacroCalls, d as defaultTransformers } from './shared/nuxt-component-meta.Cb-Q41oK.mjs';
import 'scule';
import 'oxc-parser';
import 'oxc-walker';
import 'magic-string';
  ...
  async setup(options, nuxt) {
    ...
    addTemplate({
      ...
      getContents: () => [
        ...
        ...(options.extendMetaFunctions || []).filter((f) => f.name !== "extendComponentMeta").map((f) => `declare function ${f.name}(meta: Record<string, any>): void`)
      ].join("\n"),
      write: true
    });
